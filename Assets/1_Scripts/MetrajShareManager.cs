...
public class MetrajShareManager : MonoBehaviour
{
    public void ShareKayit(string kayitAdi, string tur, string detayJson)
    {
        ...
        if (tur == "Beton")
        {
            ...
            csv.AppendLine("");
            csv.AppendLine("Ýmalat;Benzer;Adet;En;Boy;Yükseklik;Sonuç (m3)");
        ...
        else if (tur == "Kalýp")
        {
            ...
            csv.AppendLine("Kayýt Adý;" + kayitAdi + ";Genel Toplam (m2);" + toplamFormula);
            csv.AppendLine("");
            csv.AppendLine("Ýmalat;Benzer;Adet;En;Boy;Sonuç (m2)");

            for (int i = 0; i < data.satirlar.Count; i++)
            {
                var s = data.satirlar[i];
                int excelRow = i + firstDataRow;
                string formula = $"=B{excelRow}*C{excelRow}*D{excelRow}*E{excelRow}";
        ...
        {
            // Demir için ayrý JSON sýnýfý kullanýlýyor
            DemirDetayData data = JsonUtility.FromJson<DemirDetayData>(detayJson);
        ...
        new NativeShare()
            ...
            .SetText("Metraj dosyasýný paylaþýyorum.")
            .Share();
    }
}